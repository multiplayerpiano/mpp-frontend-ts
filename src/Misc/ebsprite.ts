...
export class Ebsprite {
  ...
  onresize() {
    ...
    this.canvas.width = Number($(window).width());
    this.canvas.height = Number($(window).height());
  ...
  render_loop() {
    ...
    for (var i in players) {
      ...
      if (player.walking) {
        ...
        else if (player.position.x > (<HTMLCanvasElement>this.canvas).width)
          player.position.x = (<HTMLCanvasElement>this.canvas).width;
        if (player.position.y < 0) player.position.y = 0;
        else if (player.position.y > (<HTMLCanvasElement>this.canvas).width)
          player.position.y = (<HTMLCanvasElement>this.canvas).width;
      }
    }
    ...
    for (var i in players) {
      ...
      if (img)
        this.context.drawImage(
          img,
          Math.floor(
            player.position.x - this.camera.position.x - img.width / 2
          ),
...
class SpriteProvider {
  ...
  getCurrentSprite(player: Player) {
    if (
      this.sprites &&
      (this.sprites as { [key: string]: any })[this.player.direction]
    ) {
      if (player.walking) {
        var time = Date.now() - player.updateTime;
        return (this.sprites as { [key: string]: any })[this.player.direction][
          time & 0x80 ? 0 : 1
        ];
      } else {
        return (this.sprites as { [key: string]: any })[
...
class Player {
  ...
  constructor(ebsprite: Ebsprite, id: string) {
    ...
    this.sprites =
      this.ebsprite.spriteData[
        parseInt(id, 16) % this.ebsprite.spriteData.length
      ].sprites;
  ...
  move(id: string) {
    ...
    var target = {
      x:
        (Number(part.x) / 100) *
        (<HTMLCanvasElement>this.ebsprite.canvas).width,
      y:
        (Number(this.ebsprite.client.ppl[id].y) / 100) *
        (<HTMLCanvasElement>this.ebsprite.canvas).height,
    };
    var difference = {
